...
def read(TransactionId,dataItem,TransactionTable,LogTable,Flag, fileObj):
	...
	#Check for the transaction id in transaction table, check if it already exists
    if any(TransactionTable.TransactionID == TransactionId):
		#get the row no. from the transaction table of the given transaction id
        transaction_row_index = TransactionTable[TransactionTable.TransactionID == TransactionId].index.values.astype(int)[0]
		#get the timestamp of the transaction
        timeStampOfRequestingTr = TransactionTable.at[transaction_row_index, 'TimeStamp']
        ...
        if TransactionTable.at[transaction_row_index, 'State'] == 'Aborted':
            printData = op.DLine
            printData += '\nIgnore Aborted Transaction ' + str(TransactionTable.at[transaction_row_index, 'TransactionID'])
        ...
        if TransactionTable.at[transaction_row_index, 'State'] == 'Blocked' and Flag!=True:
            TransactionTable, LogTable = abort(TransactionId, TransactionTable, LogTable, fileObj)
            printData = op.DLine
            printData += '\nAbort Transaction ' + str(TransactionTable.at[transaction_row_index, 'TransactionID'])
            printToFile(printData, fileObj)
            ...
            TransactionTable.at[transaction_row_index,'ItemsLocked'].append([dataItem,'rl'])
        ...
        if TransactionTable.at[transaction_row_index, 'State'] == 'Blocked' and Flag:
            printData = op.DLine
            printData += '\nActive again Transaction ' + str(TransactionTable.at[transaction_row_index, 'TransactionID'])
            printToFile(printData, fileObj)
...
def write(TransactionId, dataItem, Flag, TransactionTable, LogTable, fileObj):
	#check if transaction id already exits in transaction table
    if any(TransactionTable.TransactionID == TransactionId):
		#get the row no. from the transaction table of the given transaction id
        transaction_row_index = TransactionTable[TransactionTable.TransactionID == TransactionId].index.values.astype(int)[0]
		#get the timestamp of the transaction
        ...
        if TransactionTable.at[transaction_row_index, 'State'] == 'Blocked' and Flag!=True:
            printData = op.DLine
            printData += '\nIgnore Blocked Transaction ' + str(TransactionTable.at[transaction_row_index, 'TransactionID'])
            printToFile(printData, fileObj)
            return op.IGNORE, TransactionTable, LogTable
        #check if requested data item already present in lock table
            ...
            Lock_row_index = LogTable[LogTable.Items == dataItem].index.values.astype(int)[0]
            ...
            if LogTable.at[Lock_row_index, 'LockState'] == 'rl' or \
                ...
                printData = op.DLine
                printData += '\nBlock Transaction ' + str(TransactionTable.at[transaction_row_index, 'TransactionID'])
                printToFile(printData, fileObj)
                return op.IGNORE, TransactionTable, LogTable
        else:
            ...
            LogTable = LogTable.append({'Items': dataItem, 'LockState': 'wl', 'LockingTransaction': [TransactionId],
                                'TransactionsWaiting': []}, ignore_index=True)
            TransactionTable.at[transaction_row_index, 'ItemsLocked'].append([dataItem,"wl"])
            return op.SUCCESS, TransactionTable, LogTable
    return op.IGNORE, TransactionTable, LogTable
...
def wait(TransactionId, dataItem, TransactionTable, LogTable):
    ...
    TransactionTable.at[transaction_row_index, 'WaitingOn'] = dataItem
